...
function Arena:on_enter(from, level, loop, units, passives, shop_level, shop_xp, lock)
  ...
  for i, unit in ipairs(units) do
    if i == 1 then
      ...
      self.player:add_follower(
        Player {group = self.main, character = unit.character, level = unit.level, passives = self.passives, ii = i}
      )
    end
  end
  ...
  elseif (self.level - (25 * self.loop)) % 6 == 0 or self.level % 25 == 0 then
    ...
    self.t:after(
      ...
      function()
              ...
              function()
                ...
                self.t:after(
                  ...
                  function()
                    ...
                    self.t:after(
                      (8 + math.floor(self.level / 2)) * 0.1 + 0.5 + 0.75,
                      function()
        ...
        self.t:every(
          function()
            return self.start_time <= 0 and (self.boss and self.boss.dead) and
              #self.main:get_objects_by_classes(self.enemies) <= 0 and
              not self.spawning_enemies and
              not self.quitting
          end,
          function()
            self:quit()
    ...
    self.t:after(
      ...
      function()
        ...
        self.t:every(
          function()
            return #self.main:get_objects_by_classes(self.enemies) <= 0 and self.wave > self.max_waves and
              not self.quitting and
              not self.spawning_enemies
          end,
          function()
            self:quit()
          end
    ...
    if self.level == 20 and self.trailer then
      ...
      Text2 {
        ...
        sx = 0.5,
        sy = 0.5,
        lines = {{text = "[light_bg, wavy_mid]music: kubbi - ember", font = fat_font, alignment = "center"}}
      }
    end
  end
  ...
  if self.level == 1 then
    local t1 =
      ...
      sx = 0.6,
      sy = 0.6,
      lines = {{text = "[light_bg]<- or a         -> or d", font = fat_font, alignment = "center"}}
    }
    local t2 =
      Text2 {
      group = self.floor,
    ...
    self.t:after(
      ...
      function()
        if (self.level - (25 * self.loop)) % 3 == 0 and #self.passives < 8 then
          ...
          self.shop_text =
            Text(
            {{text = "[wavy_mid, fg]gold: [yellow]" .. gold, font = pixul_font, alignment = "center"}},
            global_text_tags
          )
...
function Arena:gain_gold()
  ...
  gold = gold + 10 * (self.gold_gained + self.gold_picked_up + self.interest)
end
...
function Arena:transition()
  ...
  TransitionEffect {
            ...
            {
              {
                text = "[nudge_down, " ..
                  tostring(state.dark_transitions and "fg" or "bg") ..
                    "]gold gained: " .. tostring(self.gold_gained or 0) .. " + " .. tostring(self.gold_picked_up or 0),
                font = pixul_font,
                alignment = "center",
                height_multiplier = 1.5
              },
              {
                text = "[wavy_lower, " .. tostring(state.dark_transitions and "fg" or "bg") .. "]interest: 0",
                font = pixul_font,
                alignment = "center",
                height_multiplier = 1.5
              },
              ...
              t.text:set_text(
                {
                  ...
                  {
                    text = "[nudge_down, " ..
                      tostring(state.dark_transitions and "fg" or "bg") .. "]interest: " .. tostring(self.interest or 0),
                    font = pixul_font,
                    alignment = "center",
                    height_multiplier = 1.5
                  },
                  ...
                  t.text:set_text(
                    {
                      ...
                      },
                      {
                        text = "[nudge_down, " ..
                          tostring(state.dark_transitions and "fg" or "bg") ..
                            "]total: " ..
                              tostring((self.gold_gained or 0) + (self.interest or 0) + (self.gold_picked_up or 0)),
                        font = pixul_font,
                        alignment = "center"
                      }
                    }
    ...
    text = Text(
      {
        {
          text = "[wavy_lower, " .. tostring(state.dark_transitions and "fg" or "bg") .. "]gold gained: 0 + 0",
          font = pixul_font,
          alignment = "center",
          height_multiplier = 1.5
        },
      ...
      SpawnEffect {
        ...
        action = function(x, y)
          ...
          if not pass then
            ...
            local objects =
              self.main:get_objects_in_shape(
              check_circle,
              {Seeker, EnemyCritter, Critter, Player, Sentry, Automaton, Bomb, Volcano, Saboteur, Pet, Turret}
            )
            if #objects > 0 then
          ...
          if
            random:bool(
              table.reduce(
                level_to_elite_spawn_weights[self.level],
                function(memo, v)
                  return memo + v
                end
              )
           ...
           then
            local elite_type =
              level_to_elite_spawn_types[self.level][
              random:weighted_pick(unpack(level_to_elite_spawn_weights[self.level]))
            ]
            Seeker {
              group = self.main,
              x = x,
...
function CharacterHP:draw()
  ...
  graphics.rectangle(
    ...
    2,
    2,
    self.parent.dead and bg[5] or (self.hfx.hit.f and fg[0] or _G[character_color_strings[self.parent.character]][-2]),
    2
  )
