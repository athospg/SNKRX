...
function BuyScreen:on_enter(from, level, loop, units, passives, shop_level, shop_xp)
  ...
  self.shop_text =
    Text(
    {{text = "[wavy_mid, fg]shop [fg]- gold: [yellow]" .. gold, font = pixul_font, alignment = "center"}},
    global_text_tags
  )
  self.party_text =
    Text(
  ...
  self.ng_text =
    Text({{text = "[fg]NG+" .. current_new_game_plus, font = pixul_font, alignment = "center"}}, global_text_tags)
  ...
  self.level_text =
    Text(
    {{text = "[fg]Lv." .. tostring(self.level) .. get_elite_str(self.level), font = pixul_font, alignment = "center"}},
    global_text_tags
  )
  ...
  self.tutorial_button =
    ...
    action = function()
      ...
      table.insert(
        self.tutorial_cards,
        TutorialClassIcon {group = self.tutorial, x = gw / 2 + 114, y = gh / 2 + 18, class = "warrior", units = {}}
      )
      table.insert(
        self.tutorial_cards,
        TutorialClassIcon {
          ...
          y = gh / 2 + 18,
          class = "warrior",
          units = {{character = "swordsman"}, {character = "barbarian"}, {character = "juggernaut"}}
        }
      )
      table.insert(
        self.tutorial_cards,
...
function BuyScreen:buy(character, i)
  ...
  if
    ...
    ) and gold >= character_tiers[character]
   then
    if
      table.any(
        self.units,
...
function BuyScreen:set_cards(shop_level, dont_spawn_effect, first_call)
  ...
  if first_call and locked_state then
    if locked_state.cards[1] then
      self.cards[1] =
        ShopCard {group = self.main, x = 60, y = 75, w = 80, h = 90, unit = locked_state.cards[1], parent = self, i = 1}
    end
...
function RestartButton:init(args)
  ...
  self.text =
    Text(
    {{text = "[bg10]NG+" .. tostring(current_new_game_plus), font = pixul_font, alignment = "center"}},
    global_text_tags
  )
end
  ...
  if ((self.selected and input.m1.pressed) or input.enter.pressed) and not self.transitioning then
    if #self.parent.units == 0 then
      ...
      TransitionEffect {
        ...
        x = self.x,
        y = self.y,
        color = state.dark_transitions and bg[-2] or character_colors[random:table(self.parent.units).character],
        transition_action = function()
          ...
          {
            {
              text = "[wavy, " ..
                tostring(state.dark_transitions and "fg" or "bg") ..
                  "]level " .. tostring(self.parent.level) .. "/" .. tostring(25 * (self.parent.loop + 1)),
              font = pixul_font,
              alignment = "center"
            }
          },
...
function LockButton:update(dt)
  ...
  if self.selected and input.m1.pressed then
    ...
    self.spring:pull(0.2, 200, 10)
    self.text:set_text {
      {text = "[fgm5]" .. tostring(self.parent.locked and "unlock" or "lock"), font = pixul_font, alignment = "center"}
    }
    if self.parent.locked then
      self.shape.w = 44
    else
...
function LockButton:on_mouse_exit()
  if not self.parent.locked then
    self.text:set_text {
      {text = "[bg10]" .. tostring(self.parent.locked and "unlock" or "lock"), font = pixul_font, alignment = "center"}
    }
  end
  self.selected = false
end
...
function LevelButton:init(args)
  ...
  self.max_xp =
    (self.parent.shop_level == 1 and 3) or (self.parent.shop_level == 2 and 4) or (self.parent.shop_level == 3 and 5) or
    (self.parent.shop_level == 4 and 6) or
    (self.parent.shop_level == 5 and 0)
end
  ...
  if self.selected and input.m1.pressed then
    ...
    if gold < 5 then
      ...
      if self.shop_xp >= self.max_xp then
        ...
        self.max_xp =
          (self.parent.shop_level == 1 and 3) or (self.parent.shop_level == 2 and 4) or
          (self.parent.shop_level == 3 and 5) or
          (self.parent.shop_level == 4 and 6) or
          (self.parent.shop_level == 5 and 0)
      end
      ...
      {
        {
          text = "[yellow]Lv." ..
            self.parent.shop_level ..
              "[fg] shop, XP: [yellow]" .. self.shop_xp .. "/" .. self.max_xp .. "[fg], +1 XP cost: [yellow]5",
          font = pixul_font,
          alignment = "center",
        ...
        },
        {
          text = "[fg]tier 1: " ..
            t11 ..
              "%" ..
                tostring(t11 < 10 and "  " or "") .. "                                 [fgm8]tier 1: " .. t12 .. "%",
          font = pixul_font,
          alignment = "left",
          height_multiplier = 1.25
        },
        {
          text = "[green]tier 2: " ..
            t21 ..
              "%" ..
                tostring(t21 < 10 and "  " or "") .. "                                 [fgm6]tier 2: " .. t22 .. "%",
          font = pixul_font,
          alignment = "left",
          height_multiplier = 1.25
        },
        {
          text = "[blue]tier 3: " ..
            t31 ..
              "%" ..
                tostring(t31 < 10 and "  " or "") .. "                                 [fgm4]tier 3: " .. t32 .. "%",
          font = pixul_font,
          alignment = "left",
          height_multiplier = 1.25
        },
        {
          text = "[purple]tier 4: " ..
            t41 ..
              "%" ..
                tostring(t41 < 10 and "  " or "") .. "                                 [fgm2]tier 4: " .. t42 .. "%",
          font = pixul_font,
          alignment = "left",
          height_multiplier = 1.25
        }
  ...
  if (self.selected and input.m1.pressed) or input.r.pressed then
    ...
    elseif self.parent:is(Arena) then
      if gold < 5 and not self.free_reroll then
        ...
        self.parent.shop_text:set_text {
          {text = "[fg, nudge_down]gold: [yellow, nudge_down]" .. gold, font = pixul_font, alignment = "center"}
        }
        self.free_reroll = false
        self.text =
          Text({{text = "[bg10]reroll: [yellow]5", font = pixul_font, alignment = "center"}}, global_text_tags)
      end
    end
    ...
    {
      {
        text = "[" ..
          character_color_strings[self.character] ..
            "]" .. self.character:capitalize() .. "[fg] - [yellow]Lv." .. self.level,
        font = pixul_font,
        alignment = "center",
        height_multiplier = 1.25
      },
          ...
          (self.level == 3 and "yellow" or "light_bg") ..
            "]Lv.3 [" ..
              (self.level == 3 and "fg" or "light_bg") ..
                "]Effect - " ..
                  (self.level == 3 and character_effect_names[self.character] or
                    character_effect_names_gray[self.character]),
        font = pixul_font,
        alignment = "center",
        height_multiplier = 1.25
      },
      {
        text = (self.level == 3 and character_effect_descriptions[self.character]() or
          character_effect_descriptions_gray[self.character]()),
        font = pixul_font,
        alignment = "center"
      }
    },
...
function CharacterPart:init(args)
  ...
  if self.spawn_effect then
    SpawnEffect {group = main.current.effects, x = self.x, y = self.y, color = character_colors[self.character]}
  end
  ...
  if not self.parent:is(CharacterPart) then
    ...
    if self.parent.unit_grabbed and self.parent.unit_grabbed == self then
      ...
      self.parent.characters[self.i], self.parent.characters[i] =
        self.parent.characters[i],
        self.parent.characters[self.i]
  ...
  if self.selected and input.m2.pressed and not self.just_created then
    ...
    if self.reserve then
      ...
      self.parent.party_text:set_text(
        {
          {
            text = "[wavy_mid, fg]party " .. tostring(#self.parent.units) .. "/" .. tostring(max_units),
            font = pixul_font,
            alignment = "center"
          }
        }
            ...
            "]" ..
              self.character:capitalize() ..
                "[fg] - [yellow]Lv." ..
                  self.level ..
                    "[fg], tier [yellow]" ..
                      character_tiers[self.character] .. "[fg] - sells for [yellow]" .. self:get_sale_price(),
        font = pixul_font,
        alignment = "center",
        height_multiplier = 1.25
      },
...
function CharacterPart:get_sale_price()
  ...
  total =
    total +
    ((self.level == 1 and character_tiers[self.character]) or (self.level == 2 and 2 * character_tiers[self.character]) or
      (self.level == 3 and 6 * character_tiers[self.character]) or
      0)
  if self.reserve then
...
function PassiveCard:init(args)
  ...
  self.passive_name =
    Text(
    {{text = "[fg, wavy_mid]" .. passive_names[self.passive], font = pixul_font, alignment = "center"}},
    global_text_tags
  )
...
function PassiveCard:update(dt)
  ...
  if ((self.selected and input.m1.pressed) or input[tostring(self.card_i)].pressed) and self.arena.choosing_passives then
    ...
    trigger:tween(
      0.25,
      _G,
      {slow_amount = 1, music_slow_amount = 1},
      math.linear,
      function()
        slow_amount = 1
        music_slow_amount = 1
  ...
  if self.selected and input.m1.pressed and not self.unlevellable then
    ...
    if gold < 5 then
      ...
      if self.xp >= self.max_xp then
        ...
        self.max_xp =
          (self.level == 0 and 0) or (self.level == 1 and 2) or (self.level == 2 and 3) or (self.level == 3 and 0)
                      ...
                      "/" ..
                        self.max_xp ..
                          "[fg], +1 XP cost: [yellow]5[fg], sells for: [yellow]" ..
                            tostring((self.level == 1 and 10) or (self.level == 2 and 20) or (self.level == 3 and 30)),
          font = pixul_font,
          alignment = "center",
          height_multiplier = 1.25
        },
...
function ShopCard:init(args)
  ...
  self.interact_with_mouse = true
  self.character_icon =
    CharacterIcon {group = main.current.effects, x = self.x, y = self.y - 26, character = self.unit, parent = self}
  self.class_icons = {}
...
function CharacterIcon:init(args)
  ...
  self.character_text =
    ...
    {
      {
        text = "[" .. character_color_strings[self.character] .. "]" .. string.lower(character_names[self.character]),
        font = pixul_font,
        alignment = "center"
      }
    },
    ...
    {
      {
        text = "[" ..
          character_color_strings[self.character] ..
            "]" .. self.character:capitalize() .. "[fg] - cost: [yellow]" .. self.parent.cost,
        font = pixul_font,
        alignment = "center",
        height_multiplier = 1.25
      },
...
function CharacterIcon:die(dont_spawn_effect)
  ...
  if not dont_spawn_effect then
    SpawnEffect {group = main.current.effects, x = self.x, y = self.y + 4, color = character_colors[self.character]}
  end
  if self.info_text then
    self.info_text:deactivate()
    self.info_text.dead = true
  ...
  self.info_text:activate(
    {
      {
        text = "[" ..
          class_color_strings[self.class] .. "]" .. self.class:capitalize() .. "[fg] - owned: [yellow]" .. owned,
        font = pixul_font,
        alignment = "center",
        height_multiplier = 1.25
      },
...
function ClassIcon:on_mouse_enter()
    ...
    {
      {
        text = "[" ..
          class_color_strings[self.class] ..
            "]" ..
              (self.class == "conjurer" and "Builder" or self.class:capitalize()) .. "[fg] - owned: [yellow]" .. owned,
        font = pixul_font,
        alignment = "center",
        height_multiplier = 1.25
      },
  ...
  if not self.parent:is(ShopCard) then
    for _, character in ipairs(self.parent.characters) do
      if
        table.any(
          character_classes[character.character],
          function(v)
            return v == self.class
          end
        )
